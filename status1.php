...
for (s in stats) {
  ...
  card.innerHTML =
    `<div class="card-body">
        ...
        <h2 class="p-2 card-title">${85 - stats [s]}</h5>
        <div class="progress">
          <div class="progress-bar  ${_c}" role="progressbar" style="width: ${stats [s] * (100/85)}%;" aria-valuenow="${stats [s]}" aria-valuemin="0" aria-valuemax="85">${85 - stats [s]}</div>
        </div>
      </div>`

}
document.getElementById ("count").innerText = `${total} / 1267`
document.getElementById ("count").setAttribute ("style", "width: " + (total / 1267) * 100 + "%")
</script>
